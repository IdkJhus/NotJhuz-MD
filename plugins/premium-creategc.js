...
let handler = async (m, { conn, text, isOwner }) => {
   ...
   if (!text) return m.reply('_Ingrese el nombre del grupo_')
   try{
    m.reply(wait)
 ...
 /// console.log(chalk.bold.red('Membuat Grup: ' + group.gid + '\nNama Grup: ' + text + '\n\nViolet'))
    m.reply('_Grupo Creado con éxito *' + text + '*_\n\n*Nombre:* ' + text + '\n*ID:* ' + group.gid + '\n*Link:* ' + url)
       } catch (e) {
    ...
    await m.reply(`*CREACIÓN DEL GRUPO*

\`\`\`Se ha creado el grupo @${m.sender.replace(/@.+/, '')}\`\`\`
